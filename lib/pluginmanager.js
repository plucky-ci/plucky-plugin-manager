...
class PluginManager extends EventEmitter{
  ...
  localPathTo(packageName){
    const {
      name
    } = this.getDetailsFromName(packageName);
    const paths = [
      path.resolve(this.pluginsFolder, name),
      path.resolve(this.pluginsFolder, 'node_modules', name)
    ];
    const valid = paths.filter((path)=>{
      return this.isLocal(path);
    });
    return valid.shift() || false;
  }
  ...
  loadOrInstallPlugin(plugin, callback){
    const localPath = this.localPathTo(plugin);
    if(localPath){
  ...
  get(plugins, callback){
    ...
    const installPlugin = (key, next)=>{
      ...
      return this.loadOrInstallPlugin(details, (err, {plugin}={})=>{
        if(err){
          return next(err);
        }
        return next(null, {
